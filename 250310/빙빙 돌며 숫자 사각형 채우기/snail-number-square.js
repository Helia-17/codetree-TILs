...
// (!) 조건 바꿀때 명제 확인하기
// let isWall = [0, n-1].includes(r) && [0, m-1].includes(c) => &&였지만
// let isWall = [-1, n].includes(nr) || [-1, m].includes(nc) => ||임
// (!) 행 열 길이가 다를 수 있음에 주의, 변수를 n, m 달리 써야 함
...
for (let i = 1; i <= n * m; i++) {
    ...
    nr = r + dir[curDir][0]
    nc = c + dir[curDir][1]
    let isWall = [-1, n].includes(nr) || [-1, m].includes(nc)
