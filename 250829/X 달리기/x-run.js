...
// [!] while문 사용 및 1씩 감소는 O(x)이므로 나머지와 몫 활용

while (x > 0) {
    if (x - max < 0) {
        max -= 1
    } else {
        let q = parseInt(x / max)
        s += q
        x -= max * q
    }
}
...
// [!] 최고속력 이후 처리 - 남은 거리 x를, max 이하의 속도들로 분할하는 것
// [!] 최고속력 이후 처리 - 남은 거리 x는, max부터 시작해 가능한 큰 속도를 몫(q)만큼 사용해 줄여나간다